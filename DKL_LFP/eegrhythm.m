...
function signals=get_interval_starts_ends(ud)
    ...
    for type=1:2,
        ...
        if LT,
            ...
            % get the starts and ends of potential oscillation epochs; at
            % this point, the epochs can be of arbitrary length, but must
            % be spaced at least merge_width apart
            ...
            % the length in seconds of the "intervaCutoffDuration"
			durations=signals.t(end_indx{type})-signals.t(start_indx{type});
            % durations contains the length of each potential epoch in
            % seconds
