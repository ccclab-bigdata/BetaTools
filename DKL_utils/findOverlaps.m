...
%
% varargouts:
%   overlapIdx - 2-element cell array; the first contains a vector of
%      indices into the first dimension of A that have overlaps with B
%      the second contains a vector of indices into the first dimension of
%      B that have overlaps with A
...
for iEpoch = 1 : size(A, 1)
    if any(C_markers(A(iEpoch, 1):A(iEpoch, 2)))
        overlapIdx{1} = [overlapIdx{1}, iEpoch];
    end
end

overlapIdx{2} = [];
for iEpoch = 1 : size(B, 1)
    if any(C_markers(B(iEpoch, 1):B(iEpoch, 2)))
        overlapIdx{2} = [overlapIdx{2}, iEpoch];
    end
end
