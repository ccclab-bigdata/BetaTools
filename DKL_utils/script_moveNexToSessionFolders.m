...
for iSubjectDir = 1 : length(subjectDirStruct)
    ...
    if strcmp(subjectDirStruct(iSubjectDir).name, '.') || ...
       strcmp(subjectDirStruct(iSubjectDir).name, '..') || ...
       strcmp(subjectDirStruct(iSubjectDir).name, '.DS_Store') || ...
       ~isdir(fullfile(sessionsRoot, subjectDirStruct(iSubjectDir).name))
        continue;
    end
    
    numSubjectDirs = numSubjectDirs + 1;
    
    curSubjectDir = fullfile(sessionsRoot, subjectDirStruct(iSubjectDir).name);
    cd(curSubjectDir);
    ...
    for iSessionDir = 1 : length(sessionDirStruct)
        ...
        if strcmp(sessionDirStruct(iSessionDir).name, '.') || ...
           ...
           strcmp(sessionDirStruct(iSessionDir).name, '.DS_Store') || ...
           ~isdir(fullfile(curSubjectDir, sessionDirStruct(iSessionDir).name))
            continue;
        end
        
        curSessionDir = fullfile(curSubjectDir, sessionDirStruct(iSessionDir).name);
        cd(curSessionDir);
